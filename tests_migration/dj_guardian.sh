...
psql -v ON_ERROR_STOP=1 -U $DB_USER -h $DB_HOST $NEW_DB -c \
	"TRUNCATE TABLE people_profile CASCADE;
...
# Give permissions.
sudo -u $USER \
psql $NEW_DB -c "INSERT INTO guardian_userobjectpermission(object_pk, content_type_id, permission_id, user_id)   SELECT '3355' as object_pk, django_content_type.id as ct_id, unnest(array[166, 169, 171]) as permission_id, people_profile.id AS user_id FROM people_profile INNER JOIN django_content_type ON django_content_type.model='resourcebase' AND people_profile.username='$TEST_USER'"
