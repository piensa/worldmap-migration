...
sudo -u $USER PGPASSWORD=$DB_PW psql -U $DB_USER -h $DB_HOST $OLD_DB -c \
	"CREATE OR REPLACE VIEW augmented_maps_layer AS SELECT * FROM maps_layer
	 INNER JOIN maps_layer_bbox USING (id) WHERE maps_layer_bbox.bbox_x0 > -181
	 AND maps_layer_bbox.bbox_x1 < 181 AND maps_layer_bbox.bbox_y0 > -91
	 AND maps_layer_bbox.bbox_y1 < 90"